...
if "$cygwin" || "$msys" ; then
    ...
    for arg do
        ...
        # temporary variables above, so each is in the correct position the
        # last time we loop over it.
        set -- "$@" "$arg"
        shift
    done
fi
...
# Stop when "start" is a word in the options.
if [ "$1" = "start" ] ; then
    die "ERROR: invalid value 'start' for option --(classpath|module-path)."
fi
